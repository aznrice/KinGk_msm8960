...
extern struct wcd9310_parameters audio_params[];
...
void write_hpvol(struct snd_soc_codec *codec, unsigned short level)
{
	...
	val = level;
	val |= WCD9310_HPOUTLZC;
	snd_soc_write(codec, WCD9310_ANALOGUE_OUT1_LEFT, val);

	val |= WCD9310_HPOUTVU;
	snd_soc_write(codec, WCD9310_ANALOGUE_OUT1_RIGHT, val);
}
...
void update_hpvol(struct snd_soc_codec *codec)
{
	...
	// read previous level
	val = snd_soc_read(codec, WCD9310_ANALOGUE_OUT1_LEFT);
	val &= ~(WCD9310_HPOUTVU_MASK);
	val &= ~(WCD9310_HPOUTLZC_MASK);
	val &= ~(WCD9310_HPL_MUTE_MASK);
	hp_level_old = val;
				     ...
				     struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "WM%X\n",
		       snd_soc_read(global_codec, WCD9310_SW_RESET_AND_ID));
}
